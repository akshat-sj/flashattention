...
__global__ void forward_kernel_v2_shared(const float* Q, const float* K, const float* V, int N, int d, int Tc, int Tr, int Bc, int Br, float scale, float* O, float* L) {
    int tid = threadIdx.x;
    int bid_x = blockIdx.x;
    int bid_y = blockIdx.y;
...
torch::Tensor forward(torch::Tensor Q,torch::Tensor K, torch::Tensor V){
    ...
    forward_kernel_v2_shared<<<grid_dim,block_dim,sram_size>>>(Q.data_ptr<float>(), K.data_ptr<float>(), V.data_ptr<float>(), N, d, Tc, Tr, Bc, Br, scale, O.data_ptr<float>(), L.data_ptr<float>());
